...
- **WhitePress**: `check_whitepress_status`, poi **risolvi un `project_id`** con `list_projects`. `search_portals` richiede un progetto, ma **non deve essere per forza quello del cliente**: il catalogo/prezzi sono a livello di portale, il progetto fissa lingua/paese e serve poi per l'ordine. Ordine di scelta: (1) il progetto del cliente, se esiste (match su URL/nome); (2) altrimenti un **progetto "lente"** già esistente nello **stesso mercato/lingua** del target; (3) altrimenti proponi all'utente di crearne uno nel pannello. Non bloccarti: se proprio non c'è alcun progetto usabile, dichiaralo e procedi con le altre fonti. (Nota: i prezzi sono in genere indipendenti dal progetto; se noti differenze, segnalalo.)
...
- **f) WhitePress `search_portals`** — è la fonte di scoperta più filtrabile (usa il `project_id` risolto nel sanity check, anche un progetto-lente nello stesso mercato se il cliente non ne ha uno): prima `get_portal_filters(project_id)` per gli **ID di categoria e paese**, poi `search_portals` con `project_id`, `portal_category` (tema del cliente), `portal_country` (mercato), `portal_quality='high'`, `offer_type='publication'` (e `offer_dofollow=1` se serve dofollow), `price_min/price_max`, soglie `min_domain_rating/min_trust_flow`, `per_page` alto e paginazione. Restituisce portali **già con `best_price` e metriche**. ⚠️ **1 richiesta/secondo**. Escludi i già-linkati; la pertinenza fine si conferma con Ahrefs (Fase 6).
